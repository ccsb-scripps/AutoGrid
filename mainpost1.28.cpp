/*
 ...
 $Id: mainpost1.28.cpp,v 1.74 2010/10/20 21:25:02 rhuey Exp $
...
if (use_vina_potential) {
    ...
    //             energy_lookup[atom_type[ia]][indx_r][map_ia] += delta_e
    //             e_repulsive:
    //             if (rddist<0.0){
    ...
    //             ihb = 0
    //             if (rddist<0.5){
    //                  energy_lookup[atom_type[ia]][indx_r][map_ia]+= 1*weight_hydrophobic;}
    //             else if ((0.5<rddist)&& (rdist<1.5)) {
    //                  energy_lookup[atom_type[ia]][indx_r][map_ia]+=(0.5-rddist)*weight_hydrophobic;
    //             };
...
#ifdef DEBUG
    printf("processing map number=%d\n", ia);
#endif
...
/* END NEW2 calculate dot product of bond vector with bond vector of best hbond */
                ...
                for (map_index = 0;  map_index < num_atom_maps;  map_index++) {
                    ...
                    if (gridmap[map_index].is_covalent == FALSE) {
                        if ((gridmap[map_index].is_hbonder == TRUE)||( not use_vina_potential)) {
